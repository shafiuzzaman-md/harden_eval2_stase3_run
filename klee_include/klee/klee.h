...
/* Vendored copy of KLEE's public klee.h (KLEE 3.2-pre).
 * Kept in-repo so the harnesses can be built with `-D__KLEE__` without a
 * host KLEE install, and so the stase3 Docker image — which ships the
 * `klee` binary at /opt/klee/bin/klee but not this header — can build them
 * too. See README.md ("Run KLEE on a single harness"). */
